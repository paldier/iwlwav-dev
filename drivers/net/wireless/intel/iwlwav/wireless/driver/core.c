                                     ...
                                     sta_entry *sta)
{
  ...
  mtlk_hstdb_dcdp_remove_all_by_sta(&nic->slow_ctx->hstdb, addr);
...
_mtlk_core_ap_disconnect_sta_blocked(struct nic *nic, struct ieee80211_sta * mac80211_sta)
{
  ...
  memcpy(addr.au8Addr, mac80211_sta->addr, sizeof(IEEE_ADDR));
...
int mtlk_core_init_defaults (mtlk_core_t *core)
{
  ...
  res = _mtlk_core_cfg_init_mcast(core);
                           ...
                           struct ieee80211_sta *mac80211_sta)
{
  ...
  psUmiStaAdd->u8SecondaryVapIndex = mtlk_vap_get_id(nic->vap_handle);
  ...
  memcpy(psUmiStaAdd->sAddr.au8Addr, mac80211_sta->addr, IEEE_ADDR_LEN);
  ...
  *subif = 0;
  ...
  if (mac_addr) {
    if (MESH_MODE_BACKHAUL_AP == MTLK_CORE_HOT_PATH_PDB_GET_INT(nic, CORE_DB_CORE_MESH_MODE)) {
      ...
      sta = mtlk_stadb_get_last_sta(&nic->slow_ctx->stadb);
      if (sta) {
...
_mtlk_core_get_unconnected_station (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(sta_req_data, sta_req_data_size)
    ...
    if(mtlk_hw_type_is_gen6(hw)){
      ELOG_D("CID-%04x: Gen6 not yet supported",
                mtlk_vap_get_oid(core->vap_handle));
      res = MTLK_ERR_NOT_SUPPORTED;
      goto end;
    }
  ...
  if (mtlk_vap_manager_is_sta_connected(core, &sta_core, addr)) {
    ...
    /* add station */
    {
      ...
      memcpy(psUmiStaAdd->sAddr.au8Addr, addr, IEEE_ADDR_LEN);
