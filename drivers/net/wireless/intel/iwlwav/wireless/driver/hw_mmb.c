...
mtlk_mmb_dcdp_rx (struct net_device *rxif, struct dp_subif *rx_subif, struct sk_buff *skb, int32_t len)
{
  ...
  mtlk_vap_manager_get_vap_handle(mtlk_vap_get_manager(ndev_vap_handle),
                                  vap_id,
                                  &vap_handle);
  if (vap_handle)
  {
...
wave_mmb_build_pdpool_return_list (mtlk_hw_t *hw, uint32 FirstFreePdOffset, wave_hw_pdpool_return_list_t *pdpool_return_list)
{
  ...
  while (NextFreePdOffset != pdPoolNextMask) {
    ...
    if (num < 10 || num > (pdPoolNumOfPds - 10) || NextFreePdOffset == 0 ||
      ...
      ILOG1_DDDD("num:%d, pdPoolElemAddr:%08x, pool_elem_idx:%d, NextFreePdOffset:%08x", num, pdPoolElemAddr, pool_elem_idx, NextFreePdOffset);
    }
#endif
                     ...
                     uint32 hw_chip_rev, int fast_path)
{
  ...
  const char *fw_suffixes[MTLK_CHIP_REV_CNT] = {
  ...
  {
    const char *fmt = "%s%s";
    const char *suffix = __hw_mmb_card_is_phy_dummy(hw) ? FW_PHY_TYPE_SUFFIX_DUMMY : FW_PHY_TYPE_SUFFIX_REAL;
  ...
  switch (cpu) {
    ...
    case CHI_CPU_NUM_IF:
    {
      const char* suffix =  _wave_hw_get_hostif_dc_mode(hw, _chipid_is_gen5(ccr->chip_info->id) && fast_path);
      snprintf(fw_info->fname, sizeof(fw_info->fname), FW_HOSTIF_NAME_FMT, chip_name, suffix);
      break;
    }
                               ...
                               uint32 targetCPU)
{
  ...
  req = (UmiLoggerMsgSetSeverity_t *) man_entry->payload;
  ...
  if (mtlk_mmb_dcdp_path_available(hw) /* && !skip_datapath_init*/)
  {
      ...
      if (dp_dev->dp_resources.buflist) {
        ...
        for (idx = 0, entry=dp_dev->dp_resources.buflist; idx < dp_dev->dp_resources.num_bufpools; idx++, entry++)
        {
          ...
          if (!dma_addr) {
            ILOG2_P("WARNING: failed mapping 0x%p to physical address", dma_addr);
            mtlk_df_dcdp_release_buflist(dp_dev);
            res = MTLK_ERR_NO_RESOURCES;
            goto finish;
          }
...
mtlk_hw_psdb_send_table (mtlk_hw_t *hw, mtlk_txmm_t *txmm, uint32 table_id, uint32 *wbuf, int nof_words)
{
    ...
    data = ((platformTable_t *)man_entry->payload);
...
int __MTLK_IFUNC wave_hw_cdb_antenna_cfg_req (mtlk_hw_t *hw, wave_hw_radio_band_cfg_t band_cfg)
{
  ...
  if (band_cfg != WAVE_HW_RADIO_BAND_CFG_DB_3x1 &&
    ...
    ELOG_D("Invalid band configuration: %s", band_cfg_to_str[band_cfg]);
    return MTLK_ERR_VALUE;
  }
  ...
  if ((band_cfg == WAVE_HW_RADIO_BAND_CFG_DB_3x1 && wave_hw_zwdfs_antenna_is_active(hw)) ||
    ...
    ELOG_D("ZWDFS antenna is already %s", (band_cfg == WAVE_HW_RADIO_BAND_CFG_DB_3x1) ? "enabled" : "disabled");
    return MTLK_ERR_VALUE;
  }
  ...
  for (radio_idx = 0; radio_idx < radio_descr->num_radios; radio_idx++) {
    radio = wave_radio_descr_wave_radio_get(radio_descr, radio_idx);
    MTLK_ASSERT(radio != NULL);
    ILOG0_D("Update COC antenna config for radio_idx %d", radio_idx);
    res = wave_radio_cdb_antenna_cfg_update(radio, ant_mask[radio_idx]);
    if (MTLK_ERR_OK != res) {
      ILOG2_D("COC update for radio_idx %d failed", radio_idx);
      return res;
    }
  }
...
{
  unsigned idx = channum2cssidx(ch_num);
  if (idx != CHAN_IDX_ILLEGAL) {
...
{
  if((idx != CHAN_IDX_ILLEGAL) && (idx <= NUM_TOTAL_CHANS)) {
