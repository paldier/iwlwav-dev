  ...
  struct cfg80211_beacon_data *beacon)
{
  ...
  if (NULL == bss_conf->ssid || MIB_ESSID_LENGTH < bss_conf->ssid_len) {
  ...
  int data_len)
{
  ...
  MTLK_CLPB_TRY(twt_params, twtData_size)
    ...
    nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, sizeof(*twt_params),
      twt_params);
  ...
  int data_len)
{
  ...
  MTLK_CLPB_TRY(dev_diag_res2_stats, stats_size)
    rsp_nbuf = wv_cfg80211_vendor_cmd_alloc_reply_skb(wiphy,
      sizeof(*dev_diag_res2_stats));
    if (!rsp_nbuf) {
      MTLK_CLPB_EXIT(MTLK_ERR_NO_MEM);
    }

    nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, sizeof(*dev_diag_res2_stats),
      dev_diag_res2_stats);
  ...
  int data_len)
{
  ...
  MTLK_CLPB_TRY(dev_diag_res3_stats, stats_size)
    rsp_nbuf = wv_cfg80211_vendor_cmd_alloc_reply_skb(wiphy,
      sizeof(*dev_diag_res3_stats));
    if (!rsp_nbuf) {
      MTLK_CLPB_EXIT(MTLK_ERR_NO_MEM);
    }

    nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, sizeof(*dev_diag_res3_stats),
      dev_diag_res3_stats);
  ...
  struct wireless_dev *wdev)
{
  ...
  rsp_nbuf = wv_cfg80211_vendor_cmd_alloc_reply_skb(wiphy, HE_OPERATION_LEN);
  if (!rsp_nbuf)
    return _mtlk_df_mtlk_to_linux_error_code(MTLK_ERR_NO_MEM);

  nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, HE_OPERATION_LEN, &he_operation);
  ...
  struct wireless_dev *wdev)
{
  ...
  nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, HE_NON_ADVERTISED_LEN, &he_non_advertised);
