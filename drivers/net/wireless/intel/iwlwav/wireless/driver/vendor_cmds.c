...
static int _wave_ieee80211_vendor_set_DTIMPeriod(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_BeaconPeriod(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_11dActive(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_StaKeepaliveIN(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_BasicRateSet(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_UpRescanExmpTm(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_IRBPngEnabled(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
...
static int _wave_ieee80211_vendor_set_IRBPngStatsRst(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
#ifdef CPTCFG_IWLWAV_IRB_DEBUG
...
static int _wave_ieee80211_vendor_set_InterfDetTime(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_InterfDetScan(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_ScanCalCwMasks(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_BssLoadInBeacon(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_FourAddrMode(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_set_ConfigMRCoex(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_DTIMPeriod(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_BeaconPeriod(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_11dActive(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_StaKeepaliveIN(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_NetworkMode(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_BasicRateSet(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_UpRescanExmpTm(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_IRBPngEnabled(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
#ifdef CPTCFG_IWLWAV_IRB_DEBUG
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_IRBPngStatsRst(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
#ifdef CPTCFG_IWLWAV_IRB_DEBUG
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_ScanCalCwMasks(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
static int _wave_ieee80211_vendor_get_BssLoadInBeacon(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
int _wave_ieee80211_vendor_get_FourAddrMode(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
...
int _wave_ieee80211_vendor_get_ConfigMRCoex(struct wiphy *wiphy, struct wireless_dev *wdev,
                const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
                ...
                const void *data, int data_len)
{
  ...
  /* ported version was a simple no error check wrapper */
  rsp_nbuf = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, aid_len);
  if (!rsp_nbuf)
  {
    res = MTLK_ERR_NO_MEM;
    goto end_1;
  ...
  /* ported version was a simple no error check wrapper */
  return cfg80211_vendor_cmd_reply(rsp_nbuf);
...
wiphy_vendor_command _wave_mac80211_vendor_commands[] = {
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_DTIM_PERIOD, _wave_ieee80211_vendor_set_DTIMPeriod, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_DTIM_PERIOD, _wave_ieee80211_vendor_get_DTIMPeriod, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_BEACON_PERIOD, _wave_ieee80211_vendor_set_BeaconPeriod, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_BEACON_PERIOD, _wave_ieee80211_vendor_get_BeaconPeriod, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_11AD_ACTIVE, _wave_ieee80211_vendor_set_11dActive, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_11AD_ACTIVE, _wave_ieee80211_vendor_get_11dActive, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_STA_KEEPALIVE_INTERVAL, _wave_ieee80211_vendor_set_StaKeepaliveIN, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_STA_KEEPALIVE_INTERVAL,_wave_ieee80211_vendor_get_StaKeepaliveIN, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_NETWORK_MODE, _wave_ieee80211_vendor_get_NetworkMode, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_BSS_BASIC_RATE_SET, _wave_ieee80211_vendor_set_BasicRateSet, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_BSS_BASIC_RATE_SET, _wave_ieee80211_vendor_get_BasicRateSet, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_UP_RESCAN_EXEMPTION_TIME, _wave_ieee80211_vendor_set_UpRescanExmpTm, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_UP_RESCAN_EXEMPTION_TIME, _wave_ieee80211_vendor_get_UpRescanExmpTm, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_IRB_PINGER_ENABLED, _wave_ieee80211_vendor_set_IRBPngEnabled, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_IRB_PINGER_ENABLED, _wave_ieee80211_vendor_get_IRBPngEnabled, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_IRB_PINGER_STATS, _wave_ieee80211_vendor_set_IRBPngStatsRst, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_IRB_PINGER_STATS, _wave_ieee80211_vendor_get_IRBPngStatsRst, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_INTERFER_TIMEOUTS, _wave_ieee80211_vendor_set_InterfDetTime, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_INTERFER_SCANTIMES, _wave_ieee80211_vendor_set_InterfDetScan, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_CALIB_CW_MASKS, _wave_ieee80211_vendor_set_ScanCalCwMasks, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_CALIB_CW_MASKS, _wave_ieee80211_vendor_get_ScanCalCwMasks, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_BSS_LOAD_IN_BEACON, _wave_ieee80211_vendor_set_BssLoadInBeacon, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_BSS_LOAD_IN_BEACON, _wave_ieee80211_vendor_get_BssLoadInBeacon, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_4ADDR_MODE, _wave_ieee80211_vendor_set_FourAddrMode, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_4ADDR_MODE, _wave_ieee80211_vendor_get_FourAddrMode, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  ...
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_COEX_ENABLE, _wave_ieee80211_vendor_set_EnableMRCoex, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_COEX_ENABLE, _wave_ieee80211_vendor_get_EnableMRCoex, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_SET_COEX_CFG, _wave_ieee80211_vendor_set_ConfigMRCoex, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
  VENDOR_CMD(LTQ_NL80211_VENDOR_SUBCMD_GET_COEX_CFG, _wave_ieee80211_vendor_get_ConfigMRCoex, WIPHY_VENDOR_CMD_NEED_WDEV_NETDEV),
